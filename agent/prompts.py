...
def planner_guidance(enabled_tool_names: list[str] | None = None) -> str:
    ...
    local_note = (
        "local_retrieve 收集本地电路知识库证据，KB 证据会被 rank_evidence 排序并进入最终答案；"
...
def build_final_answer_system_prompt(has_local_retrieval: bool = True) -> str:
    """Build the final answer system prompt, conditionally including KB evidence rules."""
    kb_rule = (
        "9. 本地 KB 证据使用：KB 证据来自预建技术文档索引，可能文不对题或部分过时；"
        "如与图片识别结果矛盾，采信图片并注明冲突；将 KB 内容自然地融入分析与结论，"
        '不必写明"根据本地知识库"。\n'
    )
    no_kb_rules = """10. 不得编造题面和证据中均未出现的元件标号、参数值或型号。只回答题目问的问题，不要把证据原文作为答案主体。
11. 常见题型必须覆盖的要点：
...
12. 输出中文；可以用简洁小标题、编号列表或对比表提高可读性，不输出 JSON 或代码块。"""

    if has_local_retrieval:
        suffix = kb_rule + no_kb_rules
    else:
        # Renumber rules 10-12 -> 9-11 when KB rule is absent
        suffix = no_kb_rules.replace("10. ", "9. ").replace("11. ", "10. ").replace("12. ", "11. ")

    return f"""你是电子工程组件异常分析专家，任务是基于题面和已收集证据给出可评测的中文技术答案。
...
FINAL_ANSWER_SYSTEM_PROMPT = build_final_answer_system_prompt()
...
2. 依据：用题面、图片、KB或网页证据支撑关键结论。
3. 原因机制：解释异常为什么会发生，关联题面中的元件/数值/现象。必须用具体元件标号、参数和因果链（如"R161+C24串联→容抗随频率增大而减小→高频段增益下降→抑制高频噪声放大"），禁止只写"起到滤波/补偿作用"。
...
不得编造题面和证据中均未出现的元件标号、参数值或型号；引用的证据内容应自然融入结论，不要整段搬运原文。
</输出要求>"""
